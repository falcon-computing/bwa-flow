...
void SwFPGA(std::vector<ExtParam> SwTask,int BatchNum,mem_alnreg_t *newregs)
{
  ...
  *taskNum_ptr = BatchNum;
  ...
  for (int i =0;i<BatchNum;i++){  
      int tmpidx = (int)(output_ptr[1+FPGA_RET_PARAM_NUM*2*i])<<16|
     (int)output_ptr[0+FPGA_RET_PARAM_NUM*2*i];
      newregs[tmpidx].qb = output_ptr[2+FPGA_RET_PARAM_NUM*2*i]; 
      newregs[tmpidx].rb = output_ptr[4+FPGA_RET_PARAM_NUM*2*i] + SwTask[tmpidx].rBeg;
      newregs[tmpidx].qe = output_ptr[3+FPGA_RET_PARAM_NUM*2*i] + SwTask[tmpidx].qBeg +SwTask[tmpidx].seedLength;
      newregs[tmpidx].re = output_ptr[5+FPGA_RET_PARAM_NUM*2*i ]+ SwTask[tmpidx].rBeg +SwTask[tmpidx].seedLength;
      newregs[tmpidx].score = output_ptr[6+FPGA_RET_PARAM_NUM*2*i]; 
      newregs[tmpidx].truesc = output_ptr[7+FPGA_RET_PARAM_NUM*2*i]; 
      newregs[tmpidx].w = output_ptr[8+FPGA_RET_PARAM_NUM*2*i];
    }
}
    ...
    int batch_num)
{
 ...
 // std::vector<std::vector<preResultofSw>> preResultofSw_m; 
  ...
  while(!isfinished){
    ...
    if(taskidx>=INT_MAX){                                       // start the sw compute both on fpga and cpu   
