...
mem_chain_v mem_chain_newflow(const mem_opt_t *opt, const bwt_t *bwt, const bntseq_t *bns, int len, const uint8_t *seq, smem_aux_t *aux)
{
	int i, b, e, l_rep;
...
void intv2chain(smem_aux_t *SMEM, mem_chain_v *chain)
{
    ...
    extern bseq1_t *seqs;
    int id = SMEM->id_read;
 //   bseq1_t *seqs_cur = &seqs[id];



    mem_chain_v chn ;
    chn = mem_chain_newflow(aux.opt,aux.idx->bwt,aux.idx->bns,seqs[id].l_seq,(uint8_t *)seqs[id].seq,SMEM);
    chn.n = mem_chain_flt(aux.opt, chn.n, chn.a);
	mem_flt_chained_seeds(aux.opt, aux.idx->bns, aux.idx->pac, seqs[id].l_seq, (uint8_t *)seqs[id].seq, chn.n, chn.a);
    chn.id_read = id ;
	*chain = chn;
...
int mem_sort_dedup_patch(const mem_opt_t *opt, const bntseq_t *bns, const uint8_t *pac, uint8_t *query, int n, mem_alnreg_t *a)
{
	...
	for (i = 1; i < n; ++i) {
		...
		for (j = i - 1; j >= 0 && p->rid == a[j].rid && p->rb < a[j].re + opt->max_chain_gap; --j) {
			mem_alnreg_t *q = &a[j];
			int64_t tor, oq, mr, mq;
			int score, w;
			if (q->qe == q->qb) continue; // a[j] has been excluded
			tor = q->re - p->rb; // overlap length on the reference
			oq = q->qb < p->qb? q->qe - p->qb : p->qe - q->qb; // overlap length on the query
			mr = q->re - q->rb < p->re - p->rb? q->re - q->rb : p->re - p->rb; // min ref len in alignment
			mq = q->qe - q->qb < p->qe - p->qb? q->qe - q->qb : p->qe - p->qb; // min qry len in alignment
			if (tor > opt->mask_level_redun * mr && oq > opt->mask_level_redun * mq) { // one of the hits is redundant
				if (p->score < q->score) {
					p->qe = p->qb;
					break;
				} else q->qe = q->qb;
...
mem_alnreg_v mem_align1_core_new(const mem_opt_t *opt, const bntseq_t *bns, const uint8_t *pac, int l_seq, char *seq, mem_chain_v chn)
{
	int i;
//	mem_chain_v chn;
	mem_alnreg_v regs;
...
void chain2reg(mem_chain_v *chn,mem_alnreg_v *alnreg)
{
    ...
    int id = chn->id_read;
  //  bseq1_t *seqs_cur = &seqs[id];
    alnreg[id] = mem_align1_core_new(aux.opt,aux.idx->bns,aux.idx->pac,seqs[id].l_seq,seqs[id].seq,*chn);
    alnreg[id].id_read = id;
}
