...
void SMemXCLAgent::start(Task* task, FPGAAgent* agent) {
  ...
  cl_int err = 0;
  int i_arg = 0;
  err  = clSetKernelArg(kernel_, i_arg++, sizeof(cl_mem), &env_->bwt_list_[device_.env_id][0]);
  ...
  for (int i=1; i<16; i++) {
    err |= clSetKernelArg(kernel_, i_arg++, sizeof(cl_mem), &env_->bwt_list_[device_.env_id][i]);
  }
  if (err) {
